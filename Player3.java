//code coresponds to the bot "WoundAndWait"
			...
			while (true) {
				...
				if (targetPlayerPlanets.length > 0 && myPlanets.length > 0) {
					...
					for (int i = 0 ; i < myPlanets.length ; i++) {
						...
						//get fleet size
						float mySize=Float.parseFloat(myPlanetParsing.split(" ")[5]);
						//logging fleet size for checking
						logToFile("mySize: "+mySize);
						//additional variables for the data and color of target planet
						String targetWhole="";
						String color="";
						for (int j = 0 ; j < targetPlayerPlanets.length ; j++) {
							...
							if(calculateDistance(myX, myY, targetX, targetY) < minDistance){
								...
								targetWhole=targetWholePlanets[j];
								color=targetColor;
							}
						
						}
						//getting the fleet size of the closest planet
						float targetSize=Float.parseFloat(targetWhole.split(" ")[5]);
						//if it's a neutral planet attack it
						if(color.equals("null")){
							System.out.println("A " + myPlanet + " " + target);
						}
						//else wait until my fleet size is twice as big as the closest opposing planet
						else {
							if(mySize<targetSize*2){
								continue;
							}
							if(mySize>targetSize*2){
							System.out.println("A " + myPlanet + " " + target);
