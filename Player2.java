			...
			while (true) {
				...
				if (targetPlayerPlanets.length > 0 && myPlanets.length > 0) {
					...
					for (int i = 0 ; i < myPlanets.length ; i++) {
						...
						//additional variable for the data of the target planet
						String targetWhole="";
						for (int j = 0 ; j < targetPlayerPlanets.length ; j++) {
							...
							//getting coordinates and of target planet
						...
						//if my planet isn't twice as big as the target planet, dont't attack it
						if(mySize<2*targetSize){
							continue;
						}
